...
async function deploy() {
  ...
  // 2. Pack the folder
  const functionFolder = path.join(__dirname, 'appwrite', 'functions', 'update-employee-credentials');
  const tarPath = path.join(__dirname, 'update-employee-credentials.tar.gz');
  ...
  const deployment = await functions.createDeployment(
    functionId,
    InputFile.fromPath(tarPath, 'update-employee-credentials.tar.gz'),
    true, // activate
    'index.js',
    'npm install'
  );
