...
class EmployeeWorkScheduleService {
  ...
  Future<void> updateDailySchedule(EmployeeWorkScheduleModel schedule) async {
    await AppwriteService.tablesDB.updateRow(
      ...
      rowId: schedule.id,
      data: schedule.toMap()..remove('id')..remove('company_id')..remove('employee_id'),
    );
  }

  // Fetch month schedules for a given employee
  Future<Map<String, EmployeeWorkScheduleModel>> getMonthSchedules(String companyId, String employeeId, int year, int month) async {
    ...
    for (var r in response.rows) {
      ...
      final dateOnly = '${m.workDate.year}-${m.workDate.month.toString().padLeft(2,'0')}-${m.workDate.day.toString().padLeft(2,'0')}';
      map[dateOnly] = m;
    }
    return map;
  }
  ...
  Future<void> generateMonthSchedule({
    ...
    final existingSchedules = await getMonthSchedules(companyId, employeeId, year, month);
    final daysInMonth = DateTime(year, month + 1, 0).day;
    
    String? week1ShiftId;
    String? week2ShiftId;
    if (assignment.assignmentType == 'weekly_rotation' && assignment.rotationPattern != null) {
      try {
    ...
    for (int day = 1; day <= daysInMonth; day++) {
      ...
      final dateOnly = '${currentDate.year}-${currentDate.month.toString().padLeft(2,'0')}-${currentDate.day.toString().padLeft(2,'0')}';
      ...
      if (!isRestDay) {
        if (assignment.assignmentType == 'fixed') {
          ...
          if (assignment.rotationStartDate != null) {
            final diffDays = currentDate.difference(assignment.rotationStartDate!).inDays;
            if (diffDays >= 0) {
      ...
      if (existingSchedules.containsKey(dateOnly)) {
        await AppwriteService.tablesDB.updateRow(
          ...
          data: scheduleData.toMap()..remove('id')..remove('company_id')..remove('employee_id'),
        );
      } else {
